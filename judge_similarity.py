...
cleaned_dir_dl = "./experiment_data/Denoised_DeepLearning"

# Let's test the first 50 files
clean_files = sorted(os.listdir(clean_dir))[:50]


print(f"\n{'FILENAME':<20} | {'0dB SCO`RE':<10} | {'-5dB SCORE':<10} | {'CLEANED SCORE':<10} | Denoised_DeepLearning")
...
for f in clean_files:
    ...
    cleaned_dl_p = os.path.join(cleaned_dir_dl, f"{fname_base}_noise_-5dB.wav")

    if os.path.exists(noisy_med_p) and os.path.exists(noisy_hard_p) and os.path.exists(cleaned_p) and os.path.exists(cleaned_dl_p):
        ...
        print(f"{f:<20} | {score_med:.4f}     | {score_hard:.4f}     | {score_cleaned:.4f} | {score_cleaned_dl:.4f}")
    else:
